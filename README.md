...
## IntArrayList, IntVector use cases examples

With an initial default size of 10 and a size increase of 50%, IntArrayList is more relevant when the number of elements to be stored is approximately equal to the initial default size. In this situation, the number of reallocations is more appropriate and the memory consumption is lower.

In the other case, if the system is to handle a set of elements about twice the default size of IntVector, namely 40 elements, then this time, IntVector is the best option because its growth factor of 2x will limit the number of reallocations. For a size of 60 or more, it may have a memory allocation that is too resource-intensive, but this disadvantage is neutralized by the number of reallocations that would be generated by IntArrayList in this context.
